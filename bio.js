...
function setupBio() {
  // SAFEGUARD: Use canvas 'width' & 'height' instead of raw window size
...
function windowResizedBio() {
  bioWebglLayer.resizeCanvas(width, height);
  bioMaskLayer.resizeCanvas(width, height);
  initBioSimulation(true);
}

function initBioSimulation(rebuildArrays = false) {
  // SAFEGUARD: Prevent Infinity/NaN if image failed to load
  ...
  if (rebuildArrays) {
    // SAFEGUARD: Ensure the simulation grid is at least 1x1
